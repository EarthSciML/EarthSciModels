...
3. Run each `.esm` file's inline `tests` block (ESS spec §6.6) through a
   ...
   - **Python (`tools/run_esm_inline_tests.py`):** drives the public
     ...
     as `retcode MaxIters`. `components/gaschem/stratospheric/chapman.esm`,
     where one test does it — `dense_M_perturbation` drives the air number
     density to 1e25 m^-3, and Julia's explicit default exhausts its
